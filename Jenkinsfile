		...
		stage ('Lint') {
			steps {
				sh '''
					...
					aws s3 cp \
						lint.json \
						s3://s3-score1-mdt-eter-pipeline/${MODULE_NAME}/lint/${BUILD_NUMBER}_lint_$(date +%s).json
				'''
			}
		}
		...
		stage ('Sec Scanning') {
		    steps {
				sh '''
					...
					aws s3 cp \
						sec.json \
						s3://s3-score1-mdt-eter-pipeline/${MODULE_NAME}/sec/${BUILD_NUMBER}_sec_$(date +%s).json
				'''
			}
		}
		...
		stage ('Test') {
			steps {
				sh '''
					...
					aws s3 cp \
						test.json \
						s3://s3-score1-mdt-eter-pipeline/${MODULE_NAME}/test/${BUILD_NUMBER}_test_$(date +%s).json
				'''
			}
		}
